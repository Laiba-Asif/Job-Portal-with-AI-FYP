...
export default function JobSeekerProfile() {
  ...
  // Ensure parsedData is available safely
  const parsedData = profile.parsedData?.data ?? profile.parsedData ?? {};
      ...
      <div className="bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-500 rounded-3xl shadow-lg p-10 text-white flex flex-col md:flex-row items-center gap-6">
        ...
        <div>
          <h1 className="text-3xl font-bold">
            {parsedData.personal_info?.name ?? "Unnamed User"}
          </h1>
          <p className="text-lg font-medium">
            {parsedData.job_title ?? "Job Title not provided"}
          </p>
          <p className="text-sm opacity-90">
            {parsedData.seniority ?? "N/A"} • {parsedData.years_of_experience ?? 0} years experience
          </p>
          <div className="flex gap-6 mt-3 text-sm">
            <span className="flex items-center gap-2">
              <Mail className="w-4 h-4" /> {parsedData.personal_info?.emails?.[0] ?? "No email"}
            </span>
            <span className="flex items-center gap-2">
              <Phone className="w-4 h-4" /> {parsedData.personal_info?.phones?.[0] ?? "No phone"}
            </span>
            {parsedData.links?.linkedin?.[0] && (
              <a
                href={parsedData.links.linkedin[0]}
                target="_blank"
        ...
        <Section title="Education" icon={<GraduationCap />}>
          {parsedData.education?.length ? (
            <ul className="space-y-2">
              {parsedData.education.map((edu: string, i: number) => (
        ...
        {parsedData.experience?.length ? (
          <div className="space-y-4">
            {parsedData.experience.map((exp: any, i: number) => (
              ...
              >
                <h3 className="font-semibold text-indigo-700">{exp.role ?? "Unknown role"}</h3>
                <p className="text-sm text-gray-500">
                  {exp.company ?? "Unknown company"} • {exp.start_date ?? "?"} - {exp.end_date ?? "?"}
                </p>
                <p className="text-gray-700 text-sm mt-2">{exp.description ?? "No description"}</p>
              </div>
            ))}
          </div>
        ) : (
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {parsedData.projects.map((proj: any, i: number) => (
              ...
              >
                ...
                {proj.image && <img src={proj.image} alt={proj.title} className="w-full h-40 object-cover" />}
                {proj.video && (
                  <div className="aspect-video">
                    <iframe src={proj.video} title={proj.title} className="w-full h-full" allowFullScreen />
                  </div>
                )}
              </div>
            ))}
